...
import { removeSsd, setSsd } from './systemSpecificDataManager';

export async function systemSpecificDataGetter(
  userService: UsersInterface, actionsService: ActionsInterface,
  rolesService: RolesInterface,
): Promise<void> {
  const roles = await userService.getUserRoles();

  if (roles.success) {
    setSsd('roles', roles.data);
    removeSsd('unapproved');
  }
  ...
  if (!roles.success) setSsd('unapproved', !roles.success);
}
